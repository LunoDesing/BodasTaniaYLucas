...
const previousTrackIcon = document.getElementById('previousTrack');
const nextTrackIcon = document.getElementById('nextTrack');
const title2 = document.querySelector('.music__title2');
const title3 = document.querySelector('.music__title3');

// Variable para controlar qué pista se está reproduciendo
let currentTrack = 1;

// Función para alternar entre play y pause
playPauseIcon.addEventListener('click', () => {
    const audio = currentTrack === 1 ? audio1 : audio2;
    const title = currentTrack === 1 ? title2 : title3;

    if (audio.paused) {
        audio.play();
        playPauseIcon.classList.remove('fa-play');
        playPauseIcon.classList.add('fa-pause');
        title.style.display = 'block'; // Mostrar título cuando inicia la reproducción
    } else {
        audio.pause();
...
// Función para pasar al siguiente track
nextTrackIcon.addEventListener('click', () => {
    playNextTrack();
});

// Función para volver al track anterior
previousTrackIcon.addEventListener('click', () => {
    if (currentTrack === 2) {
        audio2.pause();
        ...
        audio1.play();
        playPauseIcon.classList.remove('fa-play');
        playPauseIcon.classList.add('fa-pause'); // Cambia a pausa al reproducir la canción anterior
    }
});
...
// Eventos para reproducir la siguiente pista automáticamente cuando termine la actual
audio1.addEventListener('ended', playNextTrack);
audio2.addEventListener('ended', playNextTrack);
...
function updateTimer() {
    const targetDate = new Date("October 05, 2024 19:00:00").getTime();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const recipientNumber1 = '543885813562'; // Número para el primer botón
    const recipientNumber2 = '543885851928'; // Número para el segundo botón
